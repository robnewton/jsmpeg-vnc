...
void on_connect(server_t *server, lws *socket) { app_on_connect((app_t *)server->user, socket); }
int on_http_req(server_t *server, lws *socket, char *request) { return app_on_http_req((app_t *)server->user, socket, request); }
void on_message(server_t *server, lws *socket, void *data, size_t len) { app_on_message((app_t *)server->user, socket, data, len); }
void on_close(server_t *server, lws *socket) { app_on_close((app_t *)server->user, socket); }
...
int app_on_http_req(app_t *self, lws *socket, char *request) {
	...
	if (strcmp(request, "/") == 0) {
		lws_serve_http_file(socket, "client/index.html", "text/html; charset=utf-8", NULL, NULL);
		return true;
	}
	else if (strcmp(request, "/jsmpg.js") == 0) {
		lws_serve_http_file(socket, "client/jsmpg.js", "text/javascript; charset=utf-8", NULL, NULL);
		return true;
	}
